...
export class FileParser {
  ...
  private static extractSectionsFromText(text: string, originalFile: string): ParsedSection[] {
    ...
    for (const paragraph of paragraphs) {
      // Check if paragraph looks like a title
      const isTitle = this.looksLikeTitle(paragraph)
      ...
      if (isTitle) {
        ...
        currentSection = {
          ...
          content: '',
          level: 1,
          topic: this.detectTopic(paragraph),
          difficulty: this.detectDifficulty(paragraph),
        ...
        const cleanContent = this.cleanContent(paragraph)
    ...
    if (sections.length === 0) {
      sections.push({
        title: originalFile.replace(/\.[^/.]+$/, ''), // Remove file extension
  ...
  static async parseFile(file: File): Promise<ParsedDocument> {
    try {
      ...
      throw new Error(`Błąd parsowania pliku: ${error instanceof Error ? error.message : 'Nieznany błąd'}`)
    }
  }
  ...
  static async parseDocx(file: File): Promise<ParsedDocument> {
    try {
      ...
      return this.processSections(sections, file.name, 'docx')
    } catch (error) {
  ...
  static async parseTxt(file: File): Promise<ParsedDocument> {
    try {
      ...
      const sections = this.extractSectionsFromText(text, file.name)
      
      return this.processSections(sections, file.name, 'txt')
    } catch (error) {
      console.error('Error parsing TXT file:', error)
      throw new Error(`Błąd parsowania pliku TXT: ${error instanceof Error ? error.message : 'Nieznany błąd'}`)
    }
  }
  ...
  static async parsePdf(file: File): Promise<ParsedDocument> {
    try {
      ...
      return this.processSections(sections, file.name, 'pdf')
    } catch (error) {
      console.error('Error parsing PDF file:', error)
      throw new Error(`Błąd parsowania pliku PDF: ${error instanceof Error ? error.message : 'Nieznany błąd'}`)
    }
  }

  private static processSections(sections: ParsedSection[], originalFile: string, fileType: string): ParsedDocument {
    ...
    topicGroups.forEach((topicSections, topic) => {
      if (topicSections.length === 1) {
        ...
        const mergedSection: ParsedSection = {
          ...
          keywords: Array.from(new Set(topicSections.flatMap(s => s.keywords))),
          metadata: {
            originalFile,
    ...
    const topics = Array.from(new Set(mergedSections.map(s => s.topic).filter((topic): topic is string => Boolean(topic))))
