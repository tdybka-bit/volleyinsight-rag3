 ...
 for (const setNum of setNumbers) {
   ...
   for (const inst of setInstances.slice(0, 300)) {
     ...
     for (const [prefix, map] of [[homePrefix, homePlayers], [awayPrefix, awayPlayers]] as const) {
       ...
       // Substitution events have [playerOut, playerIn] as arrays — add BOTH
 ...
 */
 ...
 {/* ═══ SECTION 2: Set info + Mode + Languages ═══ */}
 ...
 * {favPlayer}
 ...
 {/* Live Stats — Variant A: compact sections with colored left borders */}
 {favPlayer && (() => {
   const defaultStats = {
     blocks: 0, aces: 0, attacks: 0, errors: 0, points: 0,
     serve: { sum: 0, error: 0, ace: 0 },
   ...
   return (
     <div className="bg-card border border-border rounded-xl p-4">
       ...
       <div className="space-y-2.5">
         ...
         <StatSection title="Przyjęcie" color="yellow">
   ...
   <div className="bg-card border border-border rounded-xl p-4">
     ...
     {isLoadingProfile ? (
           ...
           <div className="mt-3 pt-2 border-t border-border">
             <div className="flex items-center gap-1 text-xs text-muted-foreground">
               ...
               <span className="ml-auto text-xs">
                 {playerProfile.chunks[0]?.score > 0.7 ? '★ wysoka trafnosc' : 
                  playerProfile.chunks[0]?.score > 0.5 ? '☆ srednia trafnosc' : ''}
               </span>
             </div>
           </div>
         )}
       ...
       <div className="text-xs text-muted-foreground italic">
         <span className="block mb-1">🔍 Brak profilu w bazie wiedzy</span>
